...
export const login = async (formData) => {
    ...
    const response = await fetch('/api/login', {
        ...
        body: JSON.stringify(payload), // Send payload in body as JSON
    });

    if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || "Echec d'authentification.");
    }
};
...
export default function Login() {
    ...
    const formAction = async (event) => {
        ...
        try {
            ...
            setErrorMessage(error.message); // Set error if login fails
        } finally {
            setPending(false); // Reset the loading state
        }

        if (errorMessage) {
            // Focus on the first input if there's an error
